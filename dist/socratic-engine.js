...
class SocraticEngine {
    constructor(sessionManager, strictMode = false) {
        this.conversation = [];
        this.problem = '';
        this.sessionId = '';
    ...
    async startProblem(problem) {
        ...
        const response = await (0, error_utils_1.handleOpenAIError)(() => (0, error_utils_1.retryWithBackoff)(() => this.openai.chat.completions.create({
            model: 'gpt-4',
            messages: [
        ...
        1000, // initial delay
        'Starting problem'), 'Tutoring service');
     ...
     */
    enableAssessmentMode(expectedAnswer) {
     ...
     */
    async startAssessmentProblem(problem, expectedAnswer) {
...
Start by asking for their answer in a warm, encouraging way.`;
    ...
    checkAnswer(studentAnswer) {
        ...
        return (normalizedStudent.includes(normalizedExpected) ||
            normalizedExpected.includes(normalizedStudent) ||
            normalizedStudent === normalizedExpected ||
            // Check if both contain the same number
            (this.extractNumbers(normalizedStudent).some(num => this.extractNumbers(normalizedExpected).includes(num))));
    }
    /**
     ...
     */
    suggestPrerequisites(prerequisites) {
...
3. Try again on your own?`;
    ...
    async respondToStudent(studentInput) {
        ...
        const contextualGuidance = this.buildDetailedGuidance(assessment, nextQuestionType, studentInput);
    ...
    buildDetailedGuidance(assessment, questionType, studentInput) {
...
Now, begin with your first Socratic question to start this student's journey of discovery.`;
     ...
     */
    selectContextualQuestion(assessment, questionType, studentResponse) {
        const questionBank = {
            clarification: {
                high_confidence: [
        ...
        const confidenceLevel = assessment.confidenceLevel < 0.3
            ? 'low_confidence'
        ...
        if (questionType === SocraticQuestionType.ASSUMPTIONS && assessment.misconceptions.length > 0) {
            ...
            const questionTypeKey = questionType.replace('_', '');
            const categoryQuestions = questionBank[questionTypeKey];
            if (categoryQuestions) {
                selectedQuestions = categoryQuestions[confidenceLevel]
                    || categoryQuestions.high_confidence
    ...
    buildContextualGuidance(assessment, questionType, studentResponse = '', isUnderstandingCheck = false) {
     ...
     */
    restoreConversationHistory(interactions) {
     ...
     */
    parsePredictedConfidence(response) {
     ...
     */
    computeConfidenceDelta(currentConfidence) {
     ...
     */
    computeCalibrationError(predicted, actual) {
     ...
     */
    async scoreReasoningChain(explanation, concept) {
        try {
            const response = await this.openai.chat.completions.create({
                model: 'gpt-4',
                messages: [
...
Respond with ONLY a single integer from 0-4, nothing else.`
     ...
     */
    async assessTeachBack(explanation, concept) {
     ...
     */
    generateTransferChallenge(concept, difficulty) {
     ...
     */
    async assessTransferResponse(response, expectedApproach) {
        try {
...
Respond with ONLY "yes" if the response matches the expected approach, or "no" if it does not.`
     ...
     */
    computeBehavioralDepthLevel(assessment) {
        let level = 1;
     ...
     */
    async assessStudentQuestionQuality(question, concept) {
        try {
            const response = await this.openai.chat.completions.create({
                model: 'gpt-4',
                messages: [
