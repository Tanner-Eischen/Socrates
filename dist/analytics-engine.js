...
class AnalyticsEngine {
    ...
    calculateSuccessRate(sessions) {
        ...
        if (completedSessions.length === 0)
            return 0;
        const totalMastery = completedSessions.reduce((sum, session) => sum + (session.performance?.masteryScore || 0), 0);
    ...
    identifyKnowledgeGaps(sessions) {
        const conceptPerformance = new Map();
        sessions.forEach(session => {
            if (session.performance?.conceptsLearned) {
                session.performance.conceptsLearned.forEach((concept) => {
            ...
            if (session.performance?.struggledConcepts) {
                session.performance.struggledConcepts.forEach((concept) => {
                    if (!conceptPerformance.has(concept)) {
                        conceptPerformance.set(concept, []);
                    }
    ...
    generatePerformanceTrends(sessions, days = 30) {
        const cutoffDate = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
        const recentSessions = sessions.filter(s => new Date(s.startTime) >= cutoffDate && s.completed);
    ...
    analyzeLearningPatterns(sessions, profile) {
    ...
    generateRecommendations(analytics, patterns) {
    ...
    async generateAnalyticsReport(studentId, days = 30) {
        const profile = await data_storage_1.dataStorage.loadStudentProfile();
        ...
        const summary = {
            ...
            problemsSolved: recentSessions.filter((s) => s.completed).length,
    ...
    async updateStudentAnalytics(sessionData) {
        let profile = await data_storage_1.dataStorage.loadStudentProfile();
        if (!profile) {
            profile = data_storage_1.dataStorage.createDefaultProfile();
        }
        const sessionHistory = await data_storage_1.dataStorage.loadSessionHistory();
        ...
        await data_storage_1.dataStorage.saveStudentProfile(profile);
    }
    // Create visualization data for CLI display
    createVisualizationData(type, data) {
        switch (type) {
            case 'performance':
                return {
                    type: 'line',
                    title: 'Performance Trend',
                    data: data.performanceTrends.map((trend) => ({
    ...
    averageSessionTime(sessions) {
        if (sessions.length === 0)
            return 0;
        return this.average(sessions.map(s => s.duration || 0));
    }
    averageDifficulty(sessions) {
        if (sessions.length === 0)
            return 1;
    ...
    calculateProblemTypePreference(sessions) {
    ...
    calculateOptimalSessionLength(sessions) {
        ...
        if (sessionsByLength.length < 3)
            return 0;
        // Group by duration ranges and find the range with best average performance
        const ranges = [
            { min: 0, max: 15, sessions: [] },
            { min: 15, max: 30, sessions: [] },
            { min: 30, max: 45, sessions: [] },
            { min: 45, max: 60, sessions: [] },
            { min: 60, max: Infinity, sessions: [] }
        ];
    ...
    calculateBestPerformanceTime(sessions) {
    ...
    analyzeResponsePatterns(sessions) {
        ...
        return Array.from(conceptPatterns.entries())
            ...
            improvementTrend: this.calculateTrend(data.times)
        }))
            .slice(0, 5);
    }
    calculateTrend(values) {
        ...
        if (change > 0.1)
            return 'declining'; // Higher response time = declining
        if (change < -0.1)
            return 'improving'; // Lower response time = improving
        return 'stable';
    }
    inferLearningStyle(patterns) {
        // Simple heuristic based on response patterns and preferences
        if (patterns.responsePatterns.some((p) => p.averageTime < 30)) {
            return types_1.LearningStyle.ANALYTICAL; // Quick, analytical responses
        }
        if (patterns.optimalSessionLength > 45) {
            return types_1.LearningStyle.EXPLORATORY; // Longer sessions suggest exploratory learning
        }
        return types_1.LearningStyle.VISUAL; // Default to visual
    }
    groupByProblemType(sessions) {
        const groups = {};
        sessions.forEach(session => {
    ...
    identifyAchievements(analytics, recentSessions) {
        const achievements = [];
...
exports.analyticsEngine = AnalyticsEngine.getInstance();
