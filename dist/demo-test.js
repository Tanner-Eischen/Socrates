...
async function demonstrateSocraticEngine() {
    ...
    const problem = problem_bank_1.TEST_PROBLEMS[problemIndex];
    const description = problem_bank_1.PROBLEM_DESCRIPTIONS[problemIndex];
    ...
    try {
        ...
        const hasEncouragement = tutorResponses.some(response => /great|good|excellent|right track|thinking/i.test(response));
        const hasGuidance = tutorResponses.some(response => /what|how|why|can you|try|think about/i.test(response));
