...
class ImageProcessor {
     ...
     */
    static async callVisionAPI(imageBase64, mimeType) {
...
5. Do not include explanations or additional text`
    ...
    static calculateConfidence(text) {
        ...
        if (/\d/.test(text))
            confidence += 0.2; // Contains numbers
        if (/[+\-*/=^]/.test(text))
            confidence += 0.2; // Contains operators
        if (/[a-z]/i.test(text))
            confidence += 0.1; // Contains variables
        if (/solve|find|calculate|equation/i.test(text))
            confidence += 0.1; // Contains math keywords
        // Penalize for unclear content
        if (text.includes('?') || text.includes('unclear'))
            confidence -= 0.2;
        if (text.length < 5)
            confidence -= 0.3;
        if (text.includes('cannot') || text.includes('unable'))
            confidence -= 0.4;
     ...
     */
    static createFileMetadata(filePath) {
        const stats = fs.statSync(filePath);
        const extension = path.extname(filePath).toLowerCase();
        return {
     ...
     */
    static generateStatusMessage(result) {
        if (!result.success) {
...
ImageProcessor.SUPPORTED_FORMATS = ['png', 'jpg', 'jpeg', 'gif', 'webp'];
ImageProcessor.MAX_FILE_SIZE = 20 * 1024 * 1024; // 20MB
ImageProcessor.PROCESSING_TIMEOUT = 30000; // 30 seconds
ImageProcessor.apiKey = '';
