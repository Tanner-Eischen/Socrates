...
const study_planner_1 = require("./study-planner");
const types_1 = require("./types");
const studyPlanner = study_planner_1.StudyPlanner.getInstance();
let rl = null;
...
async function createBankProblemSession(selectedIndex) {
    const selectedProblem = problem_bank_1.TEST_PROBLEMS[selectedIndex];
    console.log(`\n📝 Problem: ${selectedProblem}`);
    console.log(`📊 Type: ${problem_bank_1.PROBLEM_DESCRIPTIONS[selectedIndex]}`);
...
async function createCustomTextSession() {
    console.log('\n📝 Custom Text Input');
    console.log('====================');
    ...
    try {
        ...
        const parsedProblem = problem_parser_1.ProblemParser.parseProblem(customText);
        if (!parsedProblem.isValid) {
            console.log('❌ Problem validation failed. Please check your input and try again.');
            return null;
        }
        // Classify the problem
        const classification = problem_classifier_1.ProblemClassifier.classify(parsedProblem);
        ...
        const preview = problem_parser_1.ProblemParser.generatePreview(parsedProblem);
...
async function createImageUploadSession() {
    ...
    try {
        ...
        const uploadedFile = image_processor_1.ImageProcessor.createFileMetadata(imagePath);
        const validationResult = image_processor_1.ImageProcessor.validateImageFile(imagePath);
        if (!validationResult.isValid) {
            console.log(`❌ Image validation failed: ${validationResult.errors.join(', ')}`);
            return null;
        }
        // Process image with OCR
        const ocrResult = await image_processor_1.ImageProcessor.processImage(imagePath);
        if (!ocrResult.success || !ocrResult.extractedText) {
            ...
            const parsedProblem = problem_parser_1.ProblemParser.parseProblem(manualText);
            if (!parsedProblem.isValid) {
                console.log('❌ Problem validation failed. Please check your input and try again.');
                return null;
            }
            ...
            console.log(`   Confidence: ${(classification.confidence * 100).toFixed(0)}%`);
            console.log(`   Estimated Time: ${classification.estimatedTime}`);
            const previewManual = problem_parser_1.ProblemParser.generatePreview(parsedProblem);
            ...
            console.log('🤖 Starting Socratic dialogue...\n');
            const engineManual = new socratic_engine_1.SocraticEngine();
            return {
                problemIndex: -1,
                problem: parsedProblem.content,
                engine: engineManual,
        ...
        const parsedProblem = problem_parser_1.ProblemParser.parseProblem(ocrResult.extractedText);
        // Classify the problem
        const classification = problem_classifier_1.ProblemClassifier.classify(parsedProblem);
        // Show analysis results
        console.log('\n📊 Problem Analysis:');
...
async function testSocraticEngine() {
    ...
    try {
        ...
        problem_bank_1.TEST_PROBLEMS.forEach((problem, index) => {
            console.log(`  ${index + 1}. ${problem} (${problem_bank_1.PROBLEM_DESCRIPTIONS[index]})`);
        });
        console.log('\n🆕 Day 2 Features:');
        console.log(`  ${problem_bank_1.TEST_PROBLEMS.length + 1}. Custom Text Input - Enter your own math problem`);
        console.log(`  ${problem_bank_1.TEST_PROBLEMS.length + 2}. Image Upload - Upload an image with a math problem`);
        console.log('\n🚀 Day 3 Advanced Features:');
        console.log(`  ${problem_bank_1.TEST_PROBLEMS.length + 3}. Analytics Dashboard - View your learning progress and insights`);
        console.log(`  ${problem_bank_1.TEST_PROBLEMS.length + 4}. Resume Session - Continue an interrupted learning session`);
        console.log(`  ${problem_bank_1.TEST_PROBLEMS.length + 5}. Study Plan Generator - Create personalized learning plans`);
        const selection = await askQuestion(`\nSelect option (1-${problem_bank_1.TEST_PROBLEMS.length + 5}): `);
        const selectedIndex = parseInt(selection) - 1;
        let session = null;
        // Handle different input types
        if (selectedIndex >= 0 && selectedIndex < problem_bank_1.TEST_PROBLEMS.length) {
            // Pre-built problem from bank
            session = await createBankProblemSession(selectedIndex);
        }
        else if (selectedIndex === problem_bank_1.TEST_PROBLEMS.length) {
            // Custom text input
            session = await createCustomTextSession();
        }
        else if (selectedIndex === problem_bank_1.TEST_PROBLEMS.length + 1) {
            // Image upload
            session = await createImageUploadSession();
        }
        else if (selectedIndex === problem_bank_1.TEST_PROBLEMS.length + 2) {
            // Analytics Dashboard
            await showAnalyticsDashboard();
            return;
        }
        else if (selectedIndex === problem_bank_1.TEST_PROBLEMS.length + 3) {
            // Resume Session
            await resumeSession();
            return;
        }
        else if (selectedIndex === problem_bank_1.TEST_PROBLEMS.length + 4) {
...
async function runDialogueLoop(session) {
    while (true) {
...
async function runValidationCheck(session) {
...
async function showAnalyticsDashboard() {
    ...
    try {
        ...
        const analyticsReport = await analytics_engine_1.analyticsEngine.generateAnalyticsReport(profile.id);
...
async function resumeSession() {
    ...
    try {
        ...
        const availableSessions = await session_manager_1.sessionManager.getResumableSessions();
        ...
        if (sessionIndex >= 0 && sessionIndex < availableSessions.length) {
            ...
            // Resume the session
            const resumedSession = await session_manager_1.sessionManager.resumeSession(sessionId);
...
async function getOrCreateStudentProfile() {
    try {
        const profileDB = await data_storage_1.dataStorage.loadStudentProfile();
        if (profileDB) {
...
async function createNewStudentProfile() {
    ...
    await data_storage_1.dataStorage.saveStudentProfile(profileDB);
    console.log(`✅ Profile created for ${name}!`);
    return profile;
}
function displayAnalyticsVisualization(analytics, profile) {
...
async function analyticsInteractiveMenu(profile) {
    console.log('\n🔧 Analytics Options:');
...
async function continueResumedSession(session) {
...
async function studyPlanInteractiveMenu(studyPlan) {
    console.log('\n🔧 Study Plan Options:');
...
async function createFocusedPlan() {
    console.log('\n🎯 Select focus area:');
    ...
    return {
        ...
        targetLevel: types_1.DifficultyLevel.ADVANCED,
        preferences: {
            difficultyProgression: 'adaptive',
            includeReview: true
        }
...
async function createCustomPlan() {
    ...
    const difficulties = [types_1.DifficultyLevel.BEGINNER, types_1.DifficultyLevel.INTERMEDIATE, types_1.DifficultyLevel.ADVANCED];
    const targetLevel = difficulties[parseInt(difficultyChoice) - 1] || types_1.DifficultyLevel.INTERMEDIATE;
    return {
...
//# sourceMappingURL=cli-tester.js.map