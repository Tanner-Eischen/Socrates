...
class AnalyticsService {
     ...
     */
    static async getSystemMetrics(timeRange) {
        try {
            let timeFilter = '';
            const queryParams = [];
            if (timeRange) {
     ...
     * Get events by type
     */
    static async getEventsByType(eventType, limit = 100, offset = 0, timeRange) {
        try {
            let timeFilter = '';
            const queryParams = [eventType, limit, offset];
            if (timeRange) {
     ...
     */
    static async getUserBehaviorPatterns(userId) {
        try {
    ...
    static calculateEngagementScore(metrics) {
        const { totalSessions, completedSessions, averageSessionDuration, totalInteractions, hintsUsed, } = metrics;
        if (totalSessions === 0)
            return 0;
     ...
     * Calculate mastery level based on difficulty progression
     */
    static calculateMasteryLevel(difficultyLevels) {
...
AnalyticsService.db = DatabaseService_1.DatabaseService;
