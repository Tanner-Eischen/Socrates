...
exports.closeDatabasePool = exports.checkDatabaseHealth = exports.seedInitialData = exports.initializeSchema = exports.createDatabasePool = void 0;
...
const createDatabasePool = (config = {}) => {
    ...
    if (process.env.DB_INMEM === 'true') {
        logger_1.logger.warn('Using in-memory database (pg-mem) for development');
    ...
    logger_1.logger.info('Creating database connection pool', {
    ...
    const pool = new pg_1.Pool(finalConfig);
    // Handle pool events
...
const initializeSchema = async (pool) => {
    ...
    try {
        ...
        try {
            ...
            logger_1.logger.info('pgcrypto extension available');
        }
        catch (extError) {
            logger_1.logger.warn('pgcrypto extension not available or cannot be created; proceeding without it', {
    ...
    `);
        ...
        `);
                ...
                logger_1.logger.warn('Skipping index creation: pg-mem does not fully support all index types');
            }
        }
        catch (indexError) {
            logger_1.logger.warn('Index creation failed; continuing without indexes', {
        ...
        `);
                ...
                logger_1.logger.warn('Skipping trigger creation: not supported in pg-mem');
            }
        }
        catch (triggerError) {
            logger_1.logger.warn('Trigger creation failed; continuing without triggers', {
        ...
        logger_1.logger.error('Failed to initialize database schema', {
...
const seedInitialData = async (pool) => {
    ...
    try {
        ...
        `, [
            ...
            logger_1.logger.warn('Admin seed step failed; continuing', { error: e?.message || e });
        }
        // Tutor user seed in isolated savepoint
        await client.query('SAVEPOINT seed_tutor');
        try {
            const tutorCheck = await client.query('SELECT id FROM users WHERE email = $1', ['tutor@socrates.com']);
            let tutorId = tutorCheck.rows[0]?.id || null;
            if (!tutorId) {
                let bcrypt;
                try {
                    bcrypt = require('bcrypt');
        ...
        `, [
                ...
                logger_1.logger.info('Tutor user created', { tutorId });
            }
            await client.query('RELEASE SAVEPOINT seed_tutor');
        }
        catch (e) {
            await client.query('ROLLBACK TO SAVEPOINT seed_tutor');
            logger_1.logger.warn('Tutor seed step failed; continuing', { error: e?.message || e });
        }
        // Student demo user seed in isolated savepoint
        await client.query('SAVEPOINT seed_student');
        let studentId;
        try {
            const studentCheck = await client.query('SELECT id FROM users WHERE email = $1', ['test@example.com']);
        ...
        `, [
                ...
                logger_1.logger.info('Student user created', { studentId });
            }
            await client.query('RELEASE SAVEPOINT seed_student');
        }
        catch (e) {
            await client.query('ROLLBACK TO SAVEPOINT seed_student');
            logger_1.logger.warn('Student seed step failed; continuing', { error: e?.message || e });
            studentId = null;
        }
        ...
        try {
            ...
            const devCheck = await client.query('SELECT id FROM users WHERE email = $1', [devEmail]);
        ...
        `, [
                ...
                logger_1.logger.info('Development user created', { devId, devEmail });
            }
            await client.query('RELEASE SAVEPOINT seed_dev');
        }
        catch (e) {
            await client.query('ROLLBACK TO SAVEPOINT seed_dev');
            logger_1.logger.warn('Dev user seed step failed; continuing', { error: e?.message || e });
        }
        ...
        `);
                problemCount = 0;
                logger_1.logger.warn('Problems table was missing during seed; created minimal schema inline');
            }
            else {
                throw e;
            }
        ...
        if (problemCount === 0) {
          ...
          `, [
                ...
                logger_1.logger.info('Sample problems created', { count: sampleProblems.length });
            }
            catch (e) {
                await client.query('ROLLBACK TO SAVEPOINT seed_problems');
                logger_1.logger.warn('Problem seeding failed; continuing without sample problems', { error: e?.message || e });
            }
        }
      ...
      `);
            const now = new Date();
            const toLevel = (d) => d === 'beginner' ? 1 : d === 'intermediate' ? 2 : 3;
            const toType = (title) => {
                if (title.toLowerCase().includes('algebra'))
                    return 'algebra';
                if (title.toLowerCase().includes('quadratic'))
                    return 'algebra';
                if (title.toLowerCase().includes('calculus'))
                    return 'calculus';
                return 'math';
            };
        ...
        `, [
                ...
                logger_1.logger.info('Demo sessions and analytics seeded for student', { studentId, count: problems.rows.length });
                await client.query('RELEASE SAVEPOINT seed_sessions_top');
            }
            catch (e) {
                await client.query('ROLLBACK TO SAVEPOINT seed_sessions_top');
                logger_1.logger.warn('Session seeding failed; continuing without demo sessions', { error: e?.message || e });
            }
        }
        await client.query('COMMIT');
        ...
        logger_1.logger.error('Seed encountered errors; proceeding in partial mode', {
            error: error instanceof Error ? error.message : 'Unknown error',
        });
    }
    finally {
...
// Graceful shutdown
const closeDatabasePool = async (pool) => {
    logger_1.logger.info('Closing database connection pool');
    try {
        await pool.end();
        logger_1.logger.info('Database connection pool closed successfully');
    }
    catch (error) {
        logger_1.logger.error('Error closing database connection pool', {
            error: error instanceof Error ? error.message : 'Unknown error',
        });
        throw error;
    }
...
exports.default = {
    ...
    initializeSchema: exports.initializeSchema,
    seedInitialData: exports.seedInitialData,
