...
import { Message, StudentProfile, SessionPerformance, LearningStyle, DifficultyLevel, AdaptiveDifficulty } from './types';
...
export class SocraticEngine {
  ...
  constructor(sessionManager?: SessionManager) {
    ...
    this.sessionManager = sessionManager || SessionManager.getInstance();
  }
  ...
  initializeSession(sessionId: string, studentProfile?: StudentProfile): void {
    ...
    if (studentProfile) {
      ...
      const adaptiveDifficulty = this.adaptiveController.calculateAdaptiveDifficulty(
        studentProfile.performanceHistory,
        studentProfile.knowledgeGaps,
  ...
  async startProblem(problem: string): Promise<string> {
    ...
    if (this.studentProfile) {
      const strategy = this.adaptiveController.generateTeachingStrategy(
        this.studentProfile.learningStyle,
        this.currentDifficulty,
        this.studentProfile.knowledgeGaps
      );
...
Start with a warm, encouraging question about what we're trying to find. Show enthusiasm for learning together!`;
  ...
  async respondToStudent(studentInput: string): Promise<string> {
    ...
    if (this.studentProfile && this.strugglingTurns > 1) {
      const recommendations = this.adaptiveController.generateRecommendations(
        this.studentProfile.performanceHistory,
        this.studentProfile.knowledgeGaps,
        this.currentDifficulty
      );
    ...
    const response = await this.openai.chat.completions.create({
      ...
      messages: [
        ...
        ...(adaptiveGuidance ? [{ role: 'system' as const, content: adaptiveGuidance }] : [])
      ],
      temperature: 0.7,
      max_tokens: 80
    });
  ...
  private trackStudentEngagement(input: string, responseTime: number): void {
  ...
  private analyzeEngagement(input: string, responseTime: number): number {
    ...
    if (input.length > 20) score += 0.1; // Detailed responses
    if (input.includes('?')) score += 0.1; // Asking questions
    if (/because|since|therefore|so/i.test(input)) score += 0.1; // Reasoning
    if (/i think|i believe|maybe|perhaps/i.test(input)) score += 0.1; // Thoughtful language
    
    return Math.max(0, Math.min(1, score));
  }

  private detectStruggling(input: string): boolean {
  ...
  private extractConceptsFromConversation(): string[] {
  ...
  private convertNumericToDifficultyLevel(numericLevel: number): DifficultyLevel {
    if (numericLevel <= 1) {
