...
export class SessionService {
  ...
  private static inMemorySessions = new Map<string, Session>();
  private static inMemoryInteractions = new Map<string, Interaction[]>();
  ...
  static async create(sessionData: CreateSessionData): Promise<Session> {
    try {
      ...
      logger.info('Learning session created (in-memory)', { 
        sessionId, 
        userId: sessionData.userId, 
        problemType: sessionData.problemType 
      });
  ...
  static async findById(id: string): Promise<Session | null> {
    try {
      ...
      if (errorCode === 'ECONNREFUSED' || errorCode === '28P01' || 
          ...
          errorMessage.includes('password authentication failed') ||
          errorMessage.includes('not available')) {
        logger.warn('Database unavailable, checking in-memory storage', { sessionId: id });
        return this.inMemorySessions.get(id) || null;
      }
  ...
  static async findByUserId(userId: string, limit: number = 20, offset: number = 0): Promise<Session[]> {
    try {
      ...
      if (errorCode === 'ECONNREFUSED' || errorCode === '28P01' || 
          errorMessage.includes('Database is not available') ||
          errorMessage.includes('not available')) {
        logger.warn('Database unavailable, checking in-memory storage', { userId });
        const sessions = Array.from(this.inMemorySessions.values())
          .filter(session => session.userId === userId)
          .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())
          .slice(offset, offset + limit);
    ...
    try {
      ...
      if (dbAvailable) {
        ...
        if (result.rows.length === 0) {
          ...
          logger.info('Session status updated (in-memory)', { sessionId: id, status, totalDuration: session.totalDuration });
          return session;
        }
        ...
        logger.info('Session status updated (in-memory)', { sessionId: id, status, totalDuration });
        return session;
      }
    } catch (error: any) {
      // If database query fails, check in-memory storage as fallback
      ...
      if (errorCode === 'ECONNREFUSED' || errorCode === '28P01' || 
        ...
        logger.info('Session status updated (in-memory fallback)', { sessionId: id, status, totalDuration });
        return session;
      }
  ...
  static async addInteraction(interactionData: CreateInteractionData): Promise<Interaction> {
    try {
      ...
      if (dbAvailable) {
        ...
        logger.info('Interaction added to session (in-memory)', { 
          interactionId, 
          sessionId: interactionData.sessionId, 
          type: interactionData.type 
        });
  ...
  static async getInteractions(sessionId: string, limit: number = 50, offset: number = 0): Promise<Interaction[]> {
    try {
      ...
      if (errorCode === 'ECONNREFUSED' || errorCode === '28P01' || 
          errorMessage.includes('Database is not available') ||
          errorMessage.includes('not available')) {
        logger.warn('Database unavailable, checking in-memory storage', { sessionId });
        const interactions = this.inMemoryInteractions.get(sessionId) || [];
        return interactions
          .sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime())
          .slice(offset, offset + limit);
  ...
  static async getSessionStats(userId: string): Promise<{
    ...
    try {
      ...
      if (errorCode === 'ECONNREFUSED' || errorCode === '28P01' || 
          errorMessage.includes('Database is not available') ||
          errorMessage.includes('not available')) {
        logger.warn('Database unavailable, calculating stats from in-memory storage', { userId });
        const sessions = Array.from(this.inMemorySessions.values())
          .filter(session => session.userId === userId);
  ...
  static async delete(id: string): Promise<void> {
    try {
      ...
      if (errorCode === 'ECONNREFUSED' || errorCode === '28P01' || 
          errorMessage.includes('Database is not available') ||
          errorMessage.includes('not available')) {
        logger.warn('Database unavailable, deleting from in-memory storage', { sessionId: id });
        if (!this.inMemorySessions.has(id)) {
          throw new Error('Session not found');
        }
        this.inMemorySessions.delete(id);
        this.inMemoryInteractions.delete(id);
        logger.info('Session deleted (in-memory fallback)', { sessionId: id });
        return;
      }
