...
const createSessionSchema = Joi.object({
  ...
  type: Joi.string().required().valid('tutoring', 'peer_learning', 'study_group', 'workshop'),
...
const sendMessageSchema = Joi.object({
  ...
  type: Joi.string().valid('text', 'voice', 'image', 'file', 'system').default('text'),
...
router.post('/sessions', 
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const mapType = (t: string): 'live_tutoring' | 'peer_learning' | 'group_study' => {
      switch (t) {
        case 'tutoring':
    ...
    logger.info('Collaboration session created via API', {
      sessionId: session.id,
      studentId: req.user!.id,
      type: session.type,
    });
...
router.get('/sessions', 
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const mapType = (t?: string): 'live_tutoring' | 'peer_learning' | 'group_study' | undefined => {
    ...
    let sessions = hostId
      ? await CollaborationService.findByUserId(hostId, limit, offset)
      : await CollaborationService.getActiveSessions();
...
router.get('/sessions/:id', 
  ...
  rateLimiter, 
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const session = await CollaborationService.findById(req.params.id);
    ...
    const participants = CollaborationService.getActiveParticipants(session.roomId);
...
router.patch('/sessions/:id', 
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    logger.info('Collaboration session updated via API', {
      sessionId: req.params.id,
      userId: req.user!.id,
      changes: ['status'],
    });
...
router.post('/sessions/:id/join', 
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const joinResult = await CollaborationService.joinSession({
    ...
    logger.info('User joined collaboration session via API', {
      sessionId: req.params.id,
      userId: req.user!.id,
      sessionType: session.type,
    });
...
router.post('/sessions/:id/leave', 
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    logger.info('User left collaboration session via API', {
      sessionId: req.params.id,
      userId: req.user!.id,
    });
