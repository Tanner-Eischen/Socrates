...
router.patch('/profile', 
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    // Remove sensitive information safely
    const userProfile = { ...(updatedUser as any) };
    delete (userProfile as any).passwordHash;
...
router.patch('/:id', 
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    logger.info('User updated by admin via API', {
...
router.post('/:id/deactivate', 
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    logger.info('User deactivated by admin via API', {
      adminUserId: req.user!.id,
      targetUserId: req.params.id,
      targetUserEmail: user.email,
    });
...
router.get('/stats', 
  ...
  analyticsRateLimiter, 
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const { users } = await UserService.getAll(1000, 0);
