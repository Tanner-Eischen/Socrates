import { SocraticEngine, SocraticQuestionType, EnhancedMessage, EnhancedStudentProfile } from './socratic-engine';
...
export class SessionManager {
  ...
  private engines: Map<string, SocraticEngine> = new Map();
  ...
  async addInteraction(sessionId: string, interaction: Omit<Interaction, 'id' | 'sessionId' | 'timestamp'>): Promise<Interaction> {
    const session = this.sessions.get(sessionId);
    if (!session) {
  ...
  async addEnhancedInteraction(sessionId: string, data: {
    type: string;
    content: string;
    confidenceLevel?: number;
    metadata?: any;
  ...
  async getSessionInteractions(sessionId: string): Promise<Interaction[]> {
    return this.interactions.get(sessionId) || [];
  }

  // Save session progress with enhanced data
  async saveSessionProgress(sessionId: string, progress: SessionProgress): Promise<void> {
    const session = this.sessions.get(sessionId);
    if (!session) return;
  ...
  async getMetacognitivePrompt(sessionId: string, category: string): Promise<string | null> {
    const engine = this.engines.get(sessionId);
    if (!engine) return null;
  ...
  private updateSessionEnhancedMetrics(session: Session, interaction: Interaction): void {
    ...
    if (metadata.targetedConcepts) {
      const newConcepts = metadata.targetedConcepts.filter(
        concept => !session.conceptsExplored.includes(concept)
      );
  ...
  async listUserSessions(userId: string, options?: {
    status?: string;
    limit?: number;
    offset?: number;
    useEnhancedEngine?: boolean;
  ...
  async completeSession(sessionId: string): Promise<Session | null> {
    ...
    if (finalAnalytics) {
      ...
      updates.questionTypesUsed = finalAnalytics.questionTypesUsed as SocraticQuestionType[];
    }
  ...
  async initializeEngineForSession(sessionId: string, studentProfile?: EnhancedStudentProfile): Promise<void> {
    const session = this.sessions.get(sessionId);
